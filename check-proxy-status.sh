...
# Print message to stdout (for normal messages) or stderr (for debug)
debug() {
    if [ "$DEBUG" = true ]; then
        echo "[DEBUG] $*" >&2
    fi
...
get_proxy_config() {
    ...
    if [ -z "${PROXY_USER}" ] || [ -z "${PROXY_PASS}" ]; then
        if [ -f "/etc/squid/.squid_passwd" ]; then
            ...
            debug "Found credentials in /etc/squid/.squid_passwd: User=$PROXY_USER"
            ...
            debug "Found credentials in /etc/squid/passwd: User=$PROXY_USER"
        fi
    fi

    # Output the configuration in a clean format (do not include any debug output)
...
if [ -n "${PORT}" ] && [ -n "${PROXY_USER}" ] && [ -n "${PROXY_PASS}" ]; then
